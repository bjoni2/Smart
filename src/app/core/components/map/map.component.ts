...
import {
  MAPTILER_API_KEY,
  MAPTILER_MAP_DEFAULT_MARKER,
  MAPTILER_MAP_STYLE,
} from 'src/app/_infrastructure/contstants';
...
export class MapComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    this.map.on('load', (map) => {
      ...
      this.mapService.boundsToNotifier
        .pipe(takeUntil(this.destroy$), startWith(this.mapPoints))
        .subscribe(() => {
          this.fitBoundsTo(this.map, this.mapPoints);
        });

      this.mapService.flyToNotifier
  ...
  public mapPoints: IMapPoint[] =
    []; /* These includes PropertyIds and Geocodes */
  ...
  getSourceJSONFrom(mapPoints: IMapPoint[]): any {
    let geojsonArray: IGeojson[] = mapPoints.map((point) => {
      ...
      geojson.geometry.coordinates = [
        parseFloat(point.geocode.Longitude),
        parseFloat(point.geocode.Latitude),
      ];
  ...
  flyTo(map: Map, mapPoint: IMapPoint): any {
    let val: maplibregl.LngLat = new maplibregl.LngLat(
      parseFloat(mapPoint.geocode.Longitude),
      parseFloat(mapPoint.geocode.Latitude)
    );
    map.flyTo({
      center: val,
      duration: 2000,
